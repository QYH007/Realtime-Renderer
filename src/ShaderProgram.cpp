...
namespace cgCourse
{
    ...
    void ShaderProgram::bind() const {
        ...
        for (auto &u:uniformsMat4fv){
            glUniformMatrix4fv(getUniformLocation(u.first), 1, GL_FALSE, &u.second[0][0]);
        }
        for (auto &u:uniformsf){
            glUniform1f(getUniformLocation(u.first), u.second);
        }
        for (auto &u:uniformsi){
            glUniform1i(getUniformLocation(u.first), u.second);
        }
        for (auto &u:uniforms2fv){
            glUniform2fv(getUniformLocation(u.first), 1, &u.second[0]);
        }
        int bindingPoint = 0;
